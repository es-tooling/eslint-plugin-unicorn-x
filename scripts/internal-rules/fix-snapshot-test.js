...
const messages = {
	[MESSAGE_ID_DISALLOWED_PROPERTY]:
		'"{{name}}" not allowed.{{autoFixEnableTip}}',
	[MESSAGE_ID_NO_SINGLE_CODE_OBJECT]:
		'Use string instead of object with "code".',
...
function getFixMarkComment(snapshotTestCall, sourceCode) {
	...
	const comment = sourceCode.getTokenBefore(snapshotTestCall, {
...
function checkInvalidCases(node, context) {
	...
	```
	*/
	const invalidCasesNode = testCasesNode.properties.find(
		(node) =>
			node.type === 'Property' &&
			!node.computed &&
			!node.method &&
	...
	switch (key.name) {
		...
		case 'code': {
			...
			if (testCase.properties.length === 1) {
				const commentsCount =
					sourceCode.getCommentsInside(testCase).length -
					...
					fix:
						commentsCount === 0
							? (fixer) =>
									fixer.replaceText(
										testCase,
										sourceCode.getText(propertyNode.value),
									)
							: undefined,
				});
			}
