...
const isEqualityCheck = (node) =>
	node.type === 'BinaryExpression' && EQUALITY_OPERATORS.has(node.operator);
const isNegatedExpression = (node) =>
	node.type === 'UnaryExpression' && node.prefix && node.operator === '!';
			...
			suggest: [
				{
					...
					*fix(fixer) {
						...
						if (
							needsSemicolon(previousToken, sourceCode, tokenAfterBang.value)
						) {
