...
const MESSAGE_ID_SUGGESTION_NOT_SPREADING_ARRAY =
	'suggestion-not-spreading-array';
...
function noArrayMutateRule(methodName) {
	const {replacement, predicate} = methods.get(methodName);
	...
	const messages = {
		...
		[MESSAGE_ID_SUGGESTION_SPREADING_ARRAY]:
			'The spreading object is an array.',
		...
		return {
			CallExpression(callExpression) {
				...
				const fixMethodName = (fixer) =>
					fixer.replaceText(methodProperty, replacement);
