...
const fixImportSpecifier = (importSpecifier, {sourceCode}) =>
	function* (fixer) {
		const declaration = importSpecifier.parent;

		yield* removeSpecifier(
			importSpecifier,
			fixer,
			sourceCode,
		...
		const hasDefaultImport = declaration.specifiers.some(
			({type}) => type === 'ImportDefaultSpecifier',
		);

		// Insert a new `ImportDeclaration`
		if (hasDefaultImport) {
			const fromToken = sourceCode.getTokenBefore(
				declaration.source,
		...
		const shouldAddComma = declaration.specifiers.some(
			(specifier) =>
				specifier !== importSpecifier &&
				specifier.type === importSpecifier.type,
		);
		yield fixer.insertTextAfter(
			importToken,
			` ${nameText}${shouldAddComma ? ',' : ''}`,
		);
	};

const fixExportSpecifier = (exportSpecifier, {sourceCode}) =>
	function* (fixer) {
