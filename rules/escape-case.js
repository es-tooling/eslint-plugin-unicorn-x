...
const getProblem = ({node, original, regex = escapeCase, lowercase, fix}) => {
	...
	if (fixed !== original) {
		return {
			...
			fix: (fixer) =>
				fix ? fix(fixer, fixed) : fixer.replaceText(node, fixed),
		};
	}
};
