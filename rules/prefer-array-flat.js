...
const messages = {
	[MESSAGE_ID]:
		'Prefer `Array#flat()` over `{{description}}` to flatten an array.',
};
	...
	testFunction(node) {
		...
		return (
			...
			(firstArgumentBody.type === 'ArrayExpression' &&
				...
				firstArgumentBody.elements.every(
					(node, index) =>
...
const emptyArrayConcat = {
	...
	getArrayNode(node) {
		...
		return argumentNode.type === 'SpreadElement'
...
const arrayPrototypeConcat = {
	testFunction(node) {
		...
		const [firstArgument, secondArgument] = node.arguments;
		return (
			isEmptyArrayExpression(firstArgument) &&
