...
test('Pass on matched token', () => {
	assert.strictEqual(assertToken(token, {
...
test('Throw error when not match', () => {
	...
	}, undefined, undefined, '`value` did not match.');
	assert.throws(() => {
		assertToken(token, {
			test: () => false,
			expected: token,
	...
	}, undefined, undefined, '`test` function return `false`.');
	assert.throws(() => {
		assertToken(token, {
			expected: {nonExistingProperty: ''},
			ruleId: 'test-rule',
		});
	}, undefined, undefined, 'assert non-existing property.');
	assert.throws(() => {
		assertToken(token, {
			expected: ['x', 'y', 'z'],
			ruleId: 'test-rule',
		});
	}, undefined, undefined, '`expected` is array.');
});
